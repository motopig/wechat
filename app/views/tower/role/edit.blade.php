...
<section class="panel panel-default">
  ...
  <div class="panel-body">
    <form method="post" action="{{ url('angel/role/doEdit') }}" class="form-horizontal frmRole">
        ...
        <div class="form-group">
          ...
          <div class="col-lg-4">
            <input type="text" name="title" class="form-control" data-required="true" value="{{$role['title']}}" placeholder="输入名称">
            ...
            <div class="col-lg-10 clkOpts">
                ...
                @foreach ($group['perms'] as $row)
                    <label class="btn btn-sm btn-info @if (! empty($row['checked'])) active @endif" data-toggle="tooltip" data-original-title="{{$row['desc']}}">
                        <input type="checkbox" name="perms[]" value="{{$row['id']}}" @if (! empty($row['checked'])) checked="checked" @endif><i class="fa fa-square text"></i><i class="fa fa-check-square text-active"></i> {{$row['title']}}
                    </label>
                @endforeach
                </div>
            </div>
...
@endforeach
        <div class="line b-b line-lg pull-in"></div>
          ...
          <div class="col-lg-8">
            <textarea name="desc" class="form-control" rows="3" placeholder="输入说明">{{$role['desc']}}</textarea>
          </div>
          </div>
        <div class="line b-b line-lg pull-in"></div>
        <div class="form-group">
          <div class="col-lg-4 col-lg-offset-2">
            <button type="button" class="btn btn-success boltClick" bolt-form="frmRole" bolt-func-success="editRole" >确认</button>&nbsp;
...
(function() {
	$(document).ready(function() {
	    ...
	    $.hell.fn.regFunc("editRole", editRole);
	});
})();
</script>
@stop